...
LocatorPopulateExternalCatalog(LocatorExternalCatalog *exCatalog)
{
	...
	exCatalog->tupDesc = *(relation->rd_att);
...
LocatorSetColumnarLayout(Oid relationOid, char *raw_string)
{
	...
	/* The target level count must be 1 or 2 */
	if (unlikely(targetLevelCount != 1 && targetLevelCount != 2))
	{
		ereport(WARNING,
	...
	{ /* Fill the layout array */
		...
		for (int i = 0; i < targetLevelCount; i++, layoutIndex++)
		{
