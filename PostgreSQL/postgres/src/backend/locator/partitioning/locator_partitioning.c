 ...
 *
 * Locator Paritioning Implementation
 *
 *
...
void
LocatorReformSiroTuple(LocatorRepartitioningDesc *desc, SiroTuple siroTuple)
{
							 ...
							 LocatorRecord locatorRecord)
{
	...
	{
		LocatorReformSiroTuple(desc, &(locatorRecord->siroTuple));
						   ...
						   bool isPartitioningWorker)
{
	...
	for (int i = 0; i < lowerInsert->nprivatebuffer; ++i)
	{
		...
		for (int i = 0; i < lowerInsert->columnGroupCount; i++)
			privateBuffer->partials[i] = false;
		...
		{
			privateBuffer->pageRecordZones[i] = ((char *) initPageRecordZones) +
				blocksPerRecordZone * BLCKSZ * i;
		}
	...
	for (;;)
	{
		...
		if (record == NULL)
		{
				...
				(upperScan->currentPartitionGenerationNumber < upperScan->maxTieredNumber))
			{
				...
				{
					/* Normal case: partitioning worker */
					upperScan->nrecordzones = upperScan->partitionLevel == 0 ?
						LocatorMempartPageCount :
						NRecordZonesUsingRecordCount( /* nblocks == nrecordzones in level 0 */
							desc->exCatalog->realRecordsCount[upperScan->partitionLevel][upperScan->partitionNumber],
							desc->parentRecordsPerRecordZone);
				}
				else
				{
					/* PostgreSQL server is shutting down: checkpointer */
					Assert(upperScan->partitionLevel == 0);
				   ...
				   int currentIndex)
{
		 ...
		 lineoff++, lpp++)
	{
		...
		VersionVisibilityStatus visibilityStatus[2] = { VERSION_UNFROZEN };
...
LocatorPartitionGetRecord(LocatorPartitionScanDesc *upperScan)
{
	...
	else
	{
			 ...
			 columnGroupIndex < columnGroupCount; columnGroupIndex++)
		{
			...
			if (++(upperScan->columnGroups[columnGroupIndex].currentIndex) ==
		 ...
		 */
		if (newPageCount == columnGroupCount)
