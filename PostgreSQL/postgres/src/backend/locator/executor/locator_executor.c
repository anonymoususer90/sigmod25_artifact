 ...
 */
static inline bool
locator_next_page_required(LocatorScanDesc pscan,
						   LocatorExecutorLevelDesc *level_desc,
						   LocatorExecutorColumnGroupDesc *group_desc)
{
					   ...
					   HeapTuple *copied_tuple)
{
	...
	/*
	 * In pandoragetpage(), if a specific tuple does not have a version visible
						  ...
						  TupleTableSlot *slot)
{
	...
	HeapScanDesc hscan = (HeapScanDesc) sscan;
	LocatorPartInfo part_info = pscan->c_part_info;
	...
	for (;;)
	{
		...
		foreach(lc, level_desc->required_column_group_desc_list)
		{
			...
			if (locator_next_page_required(pscan, level_desc, group_desc))
			{
		 ...
		 * Otherwise, move to the next tuple.
	 	 */
		if (found || (pscan->c_rec_position == pscan->c_part_info->recCnt))
			break;
	}
