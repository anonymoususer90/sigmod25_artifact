...
#include "locator/hap/hap_trigger.h"
						 ...
						 ResultRelInfo *resultRelInfo)
{
	...
	List	   *updateColnos;
#ifdef LOCATOR
	List *update_attidx_list = NIL;
	List *returning_attidx_list = NIL;
	ListCell *lc;
#endif /* LOCATOR */
	...
	foreach(lc, updateColnos)
	{
		...
		update_attidx_list = lappend_int(update_attidx_list ,
										 attnum - 1);
	}

	/*
	 * Not only the target of update but also the attribute being returned must
	 * be considered.
	 */
	foreach(lc, resultRelInfo->ri_returningList)
	{
		TargetEntry *target = lfirst(lc);

		returning_attidx_list = lappend_int(returning_attidx_list,
											target->resorigcol - 1);
	}

	/*
	 * Update uses old tuple slot at table_tuple_search_row_version().
	 */
	locator_init_columnar_slot(resultRelInfo->ri_oldTupleSlot,
							   resultRelInfo->ri_RelationDesc,
							   list_concat_copy(update_attidx_list,
												returning_attidx_list));
	
	/*
	 * Update uses new tuple slot at ExecUpdateAct().
	 */
	locator_init_columnar_slot_for_update(resultRelInfo->ri_newTupleSlot,
										  resultRelInfo->ri_RelationDesc,
										  update_attidx_list,
										  returning_attidx_list);
#endif /* LOCATOR */
...
#ifdef LOCATOR
	if (slot->tts_tuple_position != NULL &&
		locator_is_columnar_layout(slot->tts_locator_executor,
								   slot->tts_tuple_position->partitionLevel))
		locator_prepare_materialize_for_update(slot);
#endif /* LOCATOR */
...
#ifdef LOCATOR
	if (IsLocator(resultRelationDesc))
	{
		/*
		 * replace the tuple in the partition.
		 */
		result =
			table_tuple_update_for_locator_partition(resultRelationDesc, 
													 tupleid, 
...
#ifdef LOCATOR
	isLocator = IsLocator(resultRelInfo->ri_RelationDesc);
#endif /* LOCATOR */
			...
			case CMD_UPDATE:
				...
				else
				{
					...
					if (siro)
					{
						...
						if (isLocator)
						{
							Assert(slot->tts_locator_route_synopsis);

							oldSlot->tts_locator_route_synopsis =
								slot->tts_locator_route_synopsis;
						}
#endif /* LOCATOR */
				...
				if (context.planSlot)
					slot->tts_locator_route_synopsis = 
						context.planSlot->tts_locator_route_synopsis;
#endif /* LOCATOR */
			  ...
			  ResultRelInfo **insert_destrel)
{
	...
	vlock:
		...
		{
			/* Get level count */
			lvl_cnt = resultRelationDesc->rd_locator_level_count;

			/* Alloc new record info */
			locator_route_synopsis = palloc0(SizeOfLocatorRouteSynopsisData(lvl_cnt));
			...
			locator_route_synopsis->rec_key.partKey = partid;
			slot->tts_locator_route_synopsis = locator_route_synopsis;
			...
			slot->tts_locator_route_synopsis = NULL;
			pfree(locator_route_synopsis);
		}
	}
