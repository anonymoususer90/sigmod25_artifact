...
#ifdef LOCATOR
static void ExplainPrintHapQueryReduction(ExplainState *es);
#endif /* LOCATOR */
			   ...
			   const BufferUsage *bufusage)
{
	...
	if (enable_hap_reduction)
		ExplainPrintHapQueryReducitonSummary(es, queryDesc);
#endif /* LOCATOR */
...
#ifdef LOCATOR
/*
 * ExplainPrintHapQueryReducitonSummary -
 *    Print summarized query reduction
 */
void
ExplainPrintHapQueryReducitonSummary(ExplainState *es, QueryDesc *queryDesc)
{
	if (!queryDesc->plannedstmt->hapReductionPlan)
		return;

	ExplainPrintHapQueryReduction(es);
}

/*
 * ExplainPrintHapQueryReduction -
 *	  Append information about hap reduction to es->str.
 */
static void
ExplainPrintHapQueryReduction(ExplainState *es)
{
	double	reduction_time;
	bool 	has_reduction = false;

	/* Get total reduction time from PlannedStmt */
	reduction_time = es->pstmt->hapReductionTime;

	ExplainOpenGroup("QUERY_REDUCTION", "QUERY_REDUCTION", true, es);

	/* for higher density, open code the text output format */
	if (es->format == EXPLAIN_FORMAT_TEXT)
	{
		ListCell *lc;
		bool	 first_eliminated_lc = true;

		appendStringInfo(es->str, "\n");
		ExplainIndentText(es);
		appendStringInfoString(es->str, "QUERY REDUCTION:\n");

		es->indent++;
		ExplainIndentText(es);
		appendStringInfo(es->str, "Reduction Time: %.3f ms\n", reduction_time);
		
		ExplainIndentText(es);
		appendStringInfo(es->str, "Eliminated Table: ");
		...
		foreach(lc, es->rtable)
		{
			...
			if (rte->eliminated)
			{
				if (first_eliminated_lc)
					first_eliminated_lc = false;
				else
					appendStringInfo(es->str, ", ");
				...
				if (rte->alias == NULL)
					...
					appendStringInfo(es->str, "%s (alias: %s)", 
												get_rel_name(rte->relid), 
												rte->alias->aliasname);
	...
	ExplainCloseGroup("QUERY_REDUCTION", "QUERY_REDUCTION", true, es);
}
#endif /* LOCATOR */
