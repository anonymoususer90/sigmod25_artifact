...
EbiDeleteTree(dsa_pointer dsa_ebitree)
{
	...
	EbiTreeShmem->oldestSnapshot =
		*((Snapshot)dsa_get_address(ebitree_dsa_area, root->left_boundary));
#endif /* LOCATOR */
...
Snapshot
EbiGetOldestSnapshot(void)
{
	EbiTree ebitree;
	EbiNode root;

	if (unlikely(EbiTreeShmem->isShutdown))
		return &(EbiTreeShmem->oldestSnapshot);

	ebitree = EbiConvertToTree(ebitree_dsa_area, EbiTreeShmem->ebitree);
	root = EbiConvertToNode(ebitree_dsa_area, ebitree->root);

	Assert(root != NULL);
	Assert(DsaPointerIsValid(root->left_boundary));
