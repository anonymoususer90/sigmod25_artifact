...
#ifdef LOCATOR
static void locator_segfault_handler(int sig);
static void locator_abort_handler(int sig);
static pqsigfunc default_SIGSEGV;
static pqsigfunc default_SIGABRT;

static void
locator_segfault_handler(int sig)
{
...
InitProcess(void)
{
	...
	if (IsUnderPostmaster && 
			...
			!IsLocatorPartitionLauncherProcess())
#else
	...
	default_SIGSEGV = pqsignal(SIGSEGV, locator_segfault_handler);
	default_SIGABRT = pqsignal(SIGABRT, locator_abort_handler);
#endif /* LOCATOR */
}
