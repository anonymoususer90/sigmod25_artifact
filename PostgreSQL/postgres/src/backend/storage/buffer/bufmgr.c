...
static bool LocatorFlushMempartBuffer(BufferDesc *buffer, int nblock);
...
GetPrivateRefCountEntry(Buffer buffer, bool do_move)
{
	...
	if (LocatorCheckMempartBuf(buffer - 1))
		buffer = LocatorGetMempartBufId(buffer - 1) + 1;
#endif /* LOCATOR */
...
static Buffer ReadBuffer_common(SMgrRelation reln, char relpersistence,
								ForkNumber forkNum, LocatorPartLevel partLvl,
								LocatorPartNumber partNum,
								LocatorPartGenNo partGen, BlockNumber blockNum,
...
static bool FlushBuffer(BufferDesc *buf, SMgrRelation reln,
						bool isLocatorPartition);
static BufferDesc *
LocatorBufferAllocLevel0(SMgrRelation smgr, char relpersistence, ForkNumber forkNum,
						 LocatorPartLevel partLvl, LocatorPartNumber partNum,
						 LocatorPartGenNo partGen, BlockNumber blockNum,
						 BufferAccessStrategy strategy,
						 bool *foundPtr, bool is_locator_mempart_insert,
						 bool is_locator_new_mempart);
#else
static BufferDesc *BufferAlloc(SMgrRelation smgr,
							   char relpersistence,
							   ForkNumber forkNum,
 ...
 */
#ifdef LOCATOR
/* TODO: Apply locator partitioning when implementing logging & recovery */
#endif /* LOCATOR */
bool
									 ...
									 ReadBufferMode mode, BufferAccessStrategy strategy)
{
	...
	buf = ReadBuffer_common(RelationGetSmgr(reln), 'p', LOCATOR_PARTITION_FORKNUM,
						  ...
						  BufferAccessStrategy strategy, bool permanent)
{
	...
	return ReadBuffer_common(smgr, permanent ? RELPERSISTENCE_PERMANENT :
							 ...
							 InvalidLocatorPartLevel, InvalidLocatorPartNumber,
							 InvalidLocatorPartGenNo, blockNum,
							 mode, strategy, &hit);
#else
	return ReadBuffer_common(smgr, permanent ? RELPERSISTENCE_PERMANENT :
...
#endif /* LOCATOR */
{
...
#ifdef LOCATOR
	bool		isLocatorMempartInsert = false;
	bool		isLocatorPartition = (forkNum == LOCATOR_PARTITION_FORKNUM);
	bool		isLocatorNewMempart = false;
	bool		onLocatorMempartBuf = false;

	Assert(!isLocatorPartition || (
		   partLvl != InvalidLocatorPartLevel &&
		   partNum != InvalidLocatorPartNumber &&
		   partGen != InvalidLocatorPartGenNo &&
		   !isLocalBuf
	));

	/* When insert new tuple for LOCATOR, we use mempart buffer */
	if (isLocatorPartition && mode == RBM_LOCATOR_MEMPART_INSERT)
	{
		isLocatorMempartInsert = true;
		...
		else
		{
			...
			isLocatorNewMempart = true;
		}
	}

	Assert(mode != RBM_LOCATOR_MEMPART_INSERT);
#endif /* LOCATOR */
	...
	*hit = false;
	...
	{
#ifdef LOCATOR
		if (isLocatorPartition)
		{
			if (isLocatorMempartInsert)
				blockNum = 0;
			else
				blockNum = LocatorNblocks(smgr->smgr_rnode.node, partLvl, partNum,
										  partGen);
		}
		else
	...
	else
	{
		...
		if (partLvl == 0)
		{
			bufHdr = LocatorBufferAllocLevel0(smgr, relpersistence, forkNum,
							partLvl, partNum, partGen, blockNum, strategy,
							&found, isLocatorMempartInsert,
							isLocatorNewMempart);

			if (bufHdr == NULL)
			{				
				/* If this is not an insert, buffer allocation must success */
				Assert(isLocatorMempartInsert);
				return InvalidBuffer;
			}

			onLocatorMempartBuf = LocatorCheckMempartBuf(bufHdr->buf_id);
		}
		else
	...
	if (isExtend)
	{
		...
		if (onLocatorMempartBuf)
		{
			Assert(isLocatorPartition && isLocatorMempartInsert);
			if (!PageIsNew(bufBlock))
				MemSet((char *) bufBlock, 0, BLCKSZ * LocatorMempartPageCount);
		}
		else
		...
		{
			if (onLocatorMempartBuf)
				LocatorMempartExtend(smgr->smgr_rnode.node, partLvl, partNum,
									 partGen, blockNum, (char *) bufBlock);
			else
				LocatorExtend(smgr->smgr_rnode.node, partLvl, partNum, partGen,
	...
	else
	{
		...
		if (mode == RBM_ZERO_AND_LOCK || mode == RBM_ZERO_AND_CLEANUP_LOCK)
			MemSet((char *) bufBlock, 0, BLCKSZ);
#ifdef LOCATOR
		else if (mode == RBM_NO_READ_NO_LOCK)
		{
			/* Do nothing. */
		}
#endif
		else
		{
			...
			if (isLocatorPartition)
			{
				LocatorRead(smgr->smgr_rnode.node, partLvl, partNum, partGen,
							blockNum, (char *) bufBlock);
			}
			else
		...
		if (oldFlags & BM_DIRTY)
		{
										 ...
										 LW_SHARED))
			{
				...
				isWritten = FlushBuffer(buf, NULL,
										(buf->tag.forkNum == LOCATOR_PARTITION_FORKNUM));
#else
...
#ifdef LOCATOR
/* 
 * LocatorReleaseAndReadBuffer
 */
Buffer
LocatorReleaseAndReadBuffer(Buffer buffer, Relation relation,
						LocatorPartLevel partLvl, LocatorPartNumber partNum,
						LocatorPartGenNo partGen, BlockNumber blockNum)
{
	ForkNumber forkNum = LOCATOR_PARTITION_FORKNUM;
	BufferDesc *bufHdr;
...
#ifdef LOCATOR
	/* Change the given buffer to the mempart buffer if it needed */
	if (LocatorCheckMempartBuf(b - 1))
	{
		b = LocatorGetMempartBufId(b - 1) + 1;
...
#ifdef LOCATOR
/*
 * LocatorMempartSync -- Write out all dirty mempartitions in the pool.
 *
...
LocatorMempartSync(int flags)
{
	 ...
	 */
	for (buf_id = LocatorMempartStart;
		 buf_id < NBuffers + NLocatorMempartBuffers;
		 buf_id += LocatorMempartPageCount)
	{
		BufferDesc *bufHdr = GetBufferDescriptor(buf_id);
		...
		if ((buf_state & mask) == mask)
		{
			...
			mempart_id = LocatorGetMempartIdFromBufId(bufHdr->buf_id);
			Assert(mempart_id >= 0);

			/*
			 * Befor flush, get the real number of blocks in dirty mempart.
			 * Here we do not modify exCatalog's information. Just read the record
			 * count and record counts per block to calculate partition size.
			 */
			exCatalog = LocatorGetExternalCatalog(bufHdr->tag.rnode.relNode);
			exCatalogMemPartitionLock =
                LocatorExternalCatalogMemPartitionLock(exCatalog, bufHdr->tag.partNum);
			...
			if (!is_shutdown &&
				...
				exCatalog->recordNumsPerBlock * LocatorMempartPageCount)
			{
			...
			LWLockAcquire(LocatorGetMempartLock(mempart_id), LW_EXCLUSIVE);

			/* Calculate mempartition's real size */
			if (unlikely(exCatalog->CurrentTieredNumberLevelZero[bufHdr->tag.partNum] == 
														bufHdr->tag.partGen))
			{
				partition_nblock =
					NRecordZonesUsingRecordCount( /* nblocks == nrecordzones in level 0 */
						exCatalog->CurrentRecordCountsLevelZero[bufHdr->tag.partNum].val,
						exCatalog->recordNumsPerBlock);
			}
			else
				partition_nblock = LocatorMempartPageCount;

			LWLockRelease(exCatalogMemPartitionLock);

			/* Pin it, share-lock it, write it. */
			LWLockAcquire(BufferDescriptorGetContentLock(bufHdr), LW_SHARED);
			LocatorFlushMempartBuffer(bufHdr, partition_nblock);
			LWLockRelease(BufferDescriptorGetContentLock(bufHdr));

			/* Release mempart lock */
			LWLockRelease(LocatorGetMempartLock(mempart_id));
		...
		/* Check for barrier events in case LOCATOR_MEMPART_COUNT is large. */
		if (ProcSignalBarrierPending)
			ProcessProcSignalBarrier();
	}
}
...
SyncOneBuffer(int buf_id, bool skip_recently_used, WritebackContext *wb_context)
{
	...
	isWritten = FlushBuffer(bufHdr, NULL,
							(bufHdr->tag.forkNum == LOCATOR_PARTITION_FORKNUM));
#else
...
#ifdef LOCATOR
static bool
FlushBuffer(BufferDesc *buf, SMgrRelation reln, bool isLocatorPartition)
#else
static void
...
#endif /* LOCATOR */
{
	...
	/* LOCATOR's mempartition buffer has its own flush function */
	Assert(!LocatorCheckMempartBuf(buf->buf_id));
#endif /* LOCATOR */
	...
	if (isLocatorPartition)
	{
		isWritten = LocatorWrite(reln->smgr_rnode.node,
								 buf->tag.partLvl,
								 buf->tag.partNum,
								 buf->tag.partGen,
								 buf->tag.blockNum,
 ...
 */
void
ReleasePrefetchDesc(PrefetchDesc prefetch_desc, bool is_heapscan)
{
	ReadBufDesc readbuf_desc;
	PrefetchPartitionDesc partition_desc;

#ifdef LOCATOR_DEBUG
	Assert(prefetch_desc != NULL);
#endif /* LOCATOR_DEBUG */
	...
	if (LocatorSimpleClose(partition_desc->c_fd) == -1)
		ereport(ERROR, errmsg("Failed to close locator partition during prefetching"));
...
IssuePendingWritebacks(WritebackContext *context)
{
	...
	for (i = 0; i < context->nr_pending; i++)
	{
		 ...
		 */
#ifdef LOCATOR
		if (tag.forkNum == LOCATOR_PARTITION_FORKNUM)
		{
			...
			LocatorWriteback(tag.rnode, tag.partLvl, tag.partNum,
...
/* TODO: */
static BufferDesc *
BufferLookup(RelFileNode node, ForkNumber forkNum, LocatorPartLevel partLvl,
...
restart_buffer_lookup:
	...
	if (find_mempart && buf_id < LocatorMempartStart)
	{
		LWLockRelease(newPartitionLock);
	...
	while (PrefetchDescGetNeedPrefetch(prefetchDesc))
	{
		...
		for (idx = 0; idx < MAX_PAGE_IN_CHUNK; idx++)
		{
			...
			bufHdr = BufferLookup(smgr->smgr_rnode.node, MAIN_FORKNUM,
								  InvalidLocatorPartLevel,
								  InvalidLocatorPartNumber,
...
recheck_io_request:
	 ...
	 * We retrieve completed I/O from io_uring's completion queue.
	 */
	RetrieveIoCompletionFromIoUring(prefetchDesc, wait);
									   ...
									   BufferAccessStrategy strategy)
{
	...
	while (PrefetchDescGetNeedPrefetch(prefetchDesc))
	{
		...
		int 			reqIdx = 0;
		uint32			page_io_num = 0;
		bool			onLocatorMempartBuf;
		bool			prepared = false;
		bool			isNewRecZone = false;

		List			   *c_list;
		ListCell		   *c_lc;

		LocatorPartInfo		c_part_info = (LocatorPartInfo)lfirst(pdesc->c_lc);
		LocatorPartLevel	partLvl = pdesc->c_level;
		LocatorPartNumber	partNum = c_part_info->partNum;
		LocatorPartGenNo	partGen = c_part_info->partGen;

		/* Get read buffer desc from free list */
		readBufDesc = PopReadBufDescFromFreeList(prefetchDesc);

		onLocatorMempartBuf = (partLvl == 0);
		...
		{
			char			   *path;
			BlockNumber lastPageId;

#ifdef LOCATOR_DEBUG
			Assert(pdesc->c_block_cnt != 0);
#endif /* LOCATOR_DEBUG */

			/*
			 * In LOCATOR's columnar layout, prefetch is performed for each
			 * column group at each level. The last page wihin the partition may
			 * be different for each group. So do not calculate the last page in
			 * the parition for columnar layout.
			 */
			if (pdesc->is_columnar_layout)
				lastPageId = pdesc->locator_last_blocknum;
			else
				lastPageId = pdesc->c_block_cnt - 1;

			/* Get next page # to prefetch */
			reqPageId = prefetchDesc->next_reqblock;

			/*
			 * If there are no more pages to prefetch in the current partition,
			 * move to the next partition.
			 */
			if (unlikely(reqPageId > lastPageId))
			{
				if (page_io_num > 0)
				{
					reqIdx = idx - page_io_num;
					...
					/* If we can't find on buffer pool, prepare request */
					LocatorPrepareRequest(prefetchDesc->ring, pdesc->c_fd,
										  &readBufDesc->iovs[reqIdx],
										  reqIoInfo->req_page_id,
										  reqIoInfo->page_io_num, reqIoInfo,
										  true);
					prepared = true;
				...
				/* Prefetch prepared requests. */
				if (prepared)
				{
					LocatorSubmitRequest(prefetchDesc->ring);
					prepared = false;
				}
				...
				{
					/*
					 * LOCATOR manages prefetch descriptor for each column group
					 * at each level.
					 *
					 * So when we need to move to the next level, if the level
					 * uses LOCATOR's columnar layout, do not prefetch at here.
					 * Let the next level's prefetch descriptor do the work.
					 */
					if (pdesc->is_columnar_layout ||
						pdesc->c_level + 1 == pdesc->locator_columnar_start_level)
					{
						prefetchDesc->prefetch_end = true;
						goto finish;
					}
				...
				/* Close previous file */
				if (LocatorSimpleClose(pdesc->c_fd) == -1)
				{
				...
				if ((pdesc->c_fd = LocatorSimpleOpen(path, O_RDWR | PG_O_DIRECT)) == -1)
				{
				...
				/*
				 * In the case of LOCATOR's columnar layout, the logic for
				 * determining the last page and the next page to be prefetched
				 * is different from the row-oriented layout.
				 */
				if (pdesc->is_columnar_layout)
				{
					LocatorExecutorLevelDesc *level_desc
						= pdesc->locator_level_desc;
					LocatorExecutorColumnGroupDesc *group_desc
						= pdesc->locator_group_desc;

					pdesc->locator_last_blocknum
						= locator_calculate_columnar_tuple_blocknum(
								level_desc,
								group_desc,
								c_part_info->recCnt - 1); /* last record */

					pdesc->locator_current_block_idx_in_record_zone = 0;
					pdesc->locator_current_record_zone_id = 0;
					
					/*
					 * Note that the target block is in the first record zone.
					 */
					prefetchDesc->next_reqblock
			...
			/*
		 	 ...
		 	 * not currently in memory.
		 	 */
			bufHdr = BufferLookup(node, LOCATOR_PARTITION_FORKNUM, partLvl,
			...
			if (found)
			{
				...
				/* Set I/O request information */
				if (onLocatorMempartBuf && LocatorCheckMempartBuf(bufHdr->buf_id))
			...
			if (pdesc->is_columnar_layout)
			{
				LocatorExecutorLevelDesc *level_desc;
				LocatorExecutorColumnGroupDesc *group_desc;
				int column_group_block_count_in_record_zone;

				level_desc = pdesc->locator_level_desc;
				group_desc = pdesc->locator_group_desc;

				/*
				 * To check if the record zone should be moved, calculate the
				 * number of pages that can fit in the record zone.
				 */
				column_group_block_count_in_record_zone
					= (level_desc->record_count_per_record_zone
							/ group_desc->tuple_count_per_block);

				if ((level_desc->record_count_per_record_zone
						% group_desc->tuple_count_per_block) != 0)
					column_group_block_count_in_record_zone += 1;

				if (pdesc->locator_current_block_idx_in_record_zone
						== column_group_block_count_in_record_zone - 1)
				{
					/* Move to the next record zone */
					pdesc->locator_current_record_zone_id++;
				...
				{
					prefetchDesc->next_reqblock += 1;
					pdesc->locator_current_block_idx_in_record_zone += 1;
					isNewRecZone = false;
				}
			}
			else
			...
			if ((found || idx + 1 == MAX_PAGE_IN_CHUNK || isNewRecZone) &&
				page_io_num > 0)
			{
				if (found == false)
					reqIdx = idx - page_io_num + 1;
...
/*
 * BuferAlloc() for LOCATOR's level 0 inserting.
 */
static BufferDesc *
LocatorMempartBufferAllocForInsert(SMgrRelation smgr,
			char relpersistence, ForkNumber forkNum,
			LocatorPartLevel partLvl, LocatorPartNumber partNum,
			LocatorPartGenNo partGen, BlockNumber blockNum,
			BufferAccessStrategy strategy,
			bool *foundPtr, bool is_locator_new_mempart)
{
	BufferTag	newTag;			/* identity of requested block */
	uint32		newHash;		/* hash value for newTag */
	LWLock	   *newPartitionLock;	/* buffer partition lock for it */
	...
	bool		valid;
	uint32		buf_state;
	LocatorExternalCatalog *victim_exCatalog = NULL;
	int			tried = 0;
...
retry:
	 ...
	 */
	if (buf_id >= LocatorMempartStart)
	{
		/* It must be located in mempart buffer */
		Assert(buf_id >= LocatorMempartStart);

		/* The given buffer id must be a start of mempartition */
		Assert((buf_id - LocatorMempartStart) % LocatorMempartPageCount == 0);

		/* We are not creator for this mempart */
		Assert(!is_locator_new_mempart);
	 ...
	 */
	else if (!is_locator_new_mempart)
	{
		LWLockRelease(newPartitionLock);
		...
		//elog(WARNING, "LocatorMempartBufferAllocForInsert retry..\n");
	...
	for (;;)
	{
		...
		/*
		 * Select a victim buffer using LOCATOR's rule.
		 */
		buf = LocatorStrategyGetBuffer(&buf_state);

		/* It must be located in mempart buffer */
		Assert(buf->buf_id >= LocatorMempartStart);

		/* The given buffer id must be a start of mempartition */
		Assert((buf->buf_id - LocatorMempartStart) % LocatorMempartPageCount == 0);
		 ...
		 * Here we do not modify exCatalog's information. Just read the record count
		 * and record counts per block to calculate partition size.
		 */
		victim_exCatalog = LocatorGetExternalCatalog(buf->tag.rnode.relNode);

		if (victim_exCatalog != NULL)
		{
#ifdef LOCATOR_DEBUG
			fprintf(stderr, "[LOCATOR] evict, rel: %u, cnt: %u, buf: %p, pid: %d\n",
			...
			if (pg_atomic_read_u32(&(buf->inserted_cnt)) <
				victim_exCatalog->recordNumsPerBlock * LocatorMempartPageCount)
			{
				UnpinBuffer(buf, true);
				victim_exCatalog = NULL;
		...
		if (oldFlags & BM_DIRTY)
		{
			int mempart_id = LocatorGetMempartIdFromBufId(buf->buf_id);
			Assert(mempart_id >= 0);
			Assert(victim_exCatalog != NULL);

			/* Avoid inserts and updates */
			if (!LWLockConditionalAcquire(LocatorGetMempartLock(mempart_id),
										  LW_EXCLUSIVE))
			{
				UnpinBuffer(buf, true);
				victim_exCatalog = NULL;
										 ...
										 LW_SHARED))
			{
				...
				fprintf(stderr, "[LOCATOR] WARNING!!!: INSERT worker flushes dirty mempartition!!\n");
				...
				LocatorFlushMempartBuffer(buf, LocatorMempartPageCount);
				LWLockRelease(BufferDescriptorGetContentLock(buf));
		...
		if (oldFlags & BM_TAG_VALID)
		{
			 ...
			 * So victim_exCatalog cannot NULL.
			 */
			Assert(victim_exCatalog != NULL);
...
#ifdef LOCATOR_DEBUG
	fprintf(stderr, "[LOCATOR] init, rel: %u, cnt: %u, buf: %p, pid: %d\n",
...
/*
 * BufferAlloc() for LOCATOR's level0 buffers (traditional + mempart).
 */
static BufferDesc *
LocatorBufferAllocLevel0(SMgrRelation smgr, char relpersistence, ForkNumber forkNum,
			LocatorPartLevel partLvl, LocatorPartNumber partNum,
			...
			bool is_locator_new_mempart)
{
	...
	if (is_locator_mempart_insert)
		return LocatorMempartBufferAllocForInsert(smgr, relpersistence, forkNum,
										 partLvl, partNum, partGen, blockNum,
										 strategy, foundPtr,
										 is_locator_new_mempart);
	/* --------------------------------------------------------------------- */
	...
	if (buf_id >= 0)
	{
		...
		*foundPtr = true;
		...
		if (!valid)
		{
			...
			if (StartBufferIO(buf, true))
			{
				Assert(!LocatorCheckMempartBuf(buf_id));
...
/*
 * FlushBuffer() for LOCATOR's mempart
 */
bool
LocatorFlushMempartBuffer(BufferDesc *buf, int nblock)
{
	bool		isWritten = true;
	XLogRecPtr	recptr;
	ErrorContextCallback errcallback;
	 ...
	 * bufToWrite is either the shared buffer or a copy, as appropriate.
	 */
	isWritten = LocatorMempartWrite(reln->smgr_rnode.node,
							buf->tag.partLvl,
							buf->tag.partNum,
							buf->tag.partGen,
							bufBlock, nblock);
...
{
	BufferDesc *buf = GetBufferDescriptor(LocatorGetMempartBufId(buffer - 1));
...
/*
 * LocatorMempartionFlush -- Write out all dirty mempartitions in the pool.
 */
void
LocatorMempartionFlush()
{
	int			trycount = 0;
	uint32		bufState;
	int			mempartId;
	...
	uint32		mask;
	LocatorTieredNumber reorganizedTieredNumber;
...
retry_mempartition_flush:
	...
	bufHdr = LocatorStrategyGetVictimMempartitionBuffer(&bufState);
	...
	/* Get exCatalog mempartition lock */
	exCatalog = LocatorGetExternalCatalog(bufHdr->tag.rnode.relNode);
	exCatalogMemPartitionLock =
		LocatorExternalCatalogMemPartitionLock(exCatalog, bufHdr->tag.partNum);
	 ...
	 */
	LWLockAcquire(exCatalogMemPartitionLock, LW_SHARED);
	reorganizedTieredNumber =
		exCatalog->PreviousTieredNumberLevelZero[bufHdr->tag.partNum];
	LWLockRelease(exCatalogMemPartitionLock);

	/* If this mempartition is reorganization target, we skip it. */
	if (bufHdr->tag.partGen > reorganizedTieredNumber)
	{
		UnpinBuffer(bufHdr, true);
		goto retry_mempartition_flush;
	}
	...
	mempartId = LocatorGetMempartIdFromBufId(bufHdr->buf_id);
	mempartitionLock = LocatorGetMempartLock(mempartId);
	
	/* Calculate the # blocks of target mempartition. */
	nblock = LocatorMempartPageCount;
	...
	if ((bufState & mask) == mask)
	{
		...
		LWLockAcquire(BufferDescriptorGetContentLock(bufHdr), LW_SHARED);
		LocatorFlushMempartBuffer(bufHdr, nblock);
		LWLockRelease(BufferDescriptorGetContentLock(bufHdr));
	...
	/* Set zero for init. */
	MemSet(BufHdrGetBlock(bufHdr), 0, BLCKSZ * LocatorMempartPageCount);

	/* Return to free buffer. */
	LocatorStrategyFreeBuffer(bufHdr);
	UnpinBuffer(bufHdr, true);
