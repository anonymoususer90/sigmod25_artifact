							...
							off_t seekpos, int req_page_num, void *io_info)
{
	...
	fcntl(F_SETFD, vfdP->fd, PG_O_DIRECT);
...
#ifdef LOCATOR
/*
 * LocatorSimpleOpen --- same as BasicOpenFilePerm except doesn't use VFD
 *
 * This is exported for use by places that really want a plain kernel FD,
 * but need to be proof against running out of FDs.  Once an FD has been
 * successfully returned, it is the caller's responsibility to ensure that
 ...
 * 
 * Because LOCATOR partitions relation data into multiple files, there is a risk
 ...
 */
int
LocatorSimpleOpen(const char *fileName, int fileFlags)
{
	int			fd;
	...
	if (errno == EMFILE || errno == ENFILE)
	{
		ereport(LOG,
				(errcode(ERRCODE_INSUFFICIENT_RESOURCES),
				 errmsg("out of file descriptors: LOCATOR")));
	}
...
/*
 * LocatorSimpleClose --- close a file opened via LocatorSimpleOpen().
 *
 * When the backend opens a file partitioned by LOCATOR, it opens it via
 * LocatorSimpleOpen() without using the VFD abstraction level, so files opened
 * that way should also be safely closed via this function.
 * (See LocatorSimpleOpen() for details)
 */
int
LocatorSimpleClose(int fd)
{
	int result = close(fd);

	if (result != 0)
	{
		elog(LOG, "could not close file (fd: %d): LOCATOR", fd);
		return -1;
	}
