...
/*
 * fill_columnar_val
 *		Load data portion of a columnar tuple from values/isnull arrays
 *
 * NOTE: it is now REQUIRED that the caller have pre-zeroed the data area.
 */
void
fill_columnar_val(Form_pg_attribute att, char *data, Datum datum, bool isnull,
				  bool isAborted)
{
	Size		data_length;

	/* Mark aborted data. */
	if (unlikely(isAborted))
	{
		Assert(isnull);
		*data = 0x01;
		return;
			 ...
			 TupleDesc tupleDesc)
{
	 ...
	 * HeapTupleData management structure are allocated in one record zone.
	 */
	if (targetHeapTuple)
	{
