...
#ifdef LOCATOR
	if (scan->xs_heapfetch && IsLocator(scan->xs_heapfetch->rel))
		scan->lastPartitionLevel = 
			LocatorGetLevelCount(scan->xs_heapfetch->rel->rd_node.relNode) - 1;
	else
...
#ifdef LOCATOR
	if (!(scan->heapRelation->rd_locator_partitioning))
...
#ifdef LOCATOR
/* ----------------
 * index_getnext_route_synopsis - get the next route synopsis from a scan
 *
 * The result is the next route synopsis satisfying the scan keys,
 * or NULL if no more matching tuples exist.
 * ----------------
 */
LocatorRouteSynopsis
index_getnext_route_synopsis(IndexScanDesc scan, ScanDirection direction)
{
	bool		found;
	...
	/* Return the route synopsis of the tuple we found. */
	return scan->xs_route_synopsis;
}
#endif 
...
{
#ifdef LOCATOR
	bool isLocator = IsLocator(scan->heapRelation);
	...
	for (;;)
	{
		...
		if (!scan->xs_heap_continue)
		{
			if (isLocator)
			{
				routeSynopsis = index_getnext_route_synopsis(scan, direction);
				if (routeSynopsis == NULL)
					break;
				
				slot->tts_locator_route_synopsis = routeSynopsis;
			}
			else
			{
				ItemPointer tid;
