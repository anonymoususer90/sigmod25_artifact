...
#ifdef LOCATOR
/*
 * HeapTupleGetVisibilityStatus
 * Additionally set hint bits
 */
enum VersionVisibilityStatus
HeapTupleGetVisibilityStatus(HeapTuple htup, Buffer buffer)
{
	HeapTupleHeader tuple = htup->t_data;
	Snapshot snapshot = EbiGetOldestSnapshot();
	...
	/* xmin is committed, but maybe not according to the oldest snapshot */
	if (!HeapTupleHeaderXminFrozen(tuple) &&
		XidInMVCCSnapshotForEBI(HeapTupleHeaderGetRawXmin(tuple), snapshot))
		return VERSION_UNFROZEN;		/* treat as still in progress */

	/* by here, the inserting transaction has committed */

	if (tuple->t_infomask & HEAP_XMAX_INVALID)	/* xid invalid or aborted */
		return VERSION_FROZEN;

	if (HEAP_XMAX_IS_LOCKED_ONLY(tuple->t_infomask))
		return VERSION_UNFROZEN;
	...
	if (tuple->t_infomask & HEAP_XMAX_IS_MULTI)
	{
		...
		if (XidInMVCCSnapshotForEBI(xmax, snapshot))
			return VERSION_UNFROZEN;
		if (TransactionIdDidCommit(xmax))
			return VERSION_INVISIBLE;		/* updating transaction committed */
		/* it must have aborted or crashed */
		return VERSION_FROZEN;
	}
	...
	if (!(tuple->t_infomask & HEAP_XMAX_COMMITTED))
	{
		if (XidInMVCCSnapshotForEBI(HeapTupleHeaderGetRawXmax(tuple), snapshot))
			return VERSION_UNFROZEN;
