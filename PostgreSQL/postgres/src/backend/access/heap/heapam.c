 ...
 */
static void
initlocatorscan(LocatorScanDesc pscan)
{
	...
	/* Init level 0 */
	if (unlikely(pscan->part_infos_to_scan[0] != NIL))
	{
	...
	for (int i = 0; i < pscan->level_zero_part_cnt; ++i)
	{
		int part_num_level_zero = pscan->level_zero_part_nums[i];
 ...
 */
PrefetchDesc
locator_create_prefetch_desc(Relation relation, LocatorScanDesc pscan)
{
	...
	/* Init io_uring queue */
	res = io_uring_queue_init(MAX_HEAPSCAN_PREFETCH_SIZE * 2, ring,
	...
	else
	{
		...
		if ((partition_desc->c_fd = LocatorSimpleOpen(path, O_RDWR | PG_O_DIRECT)) == -1)
		{
#ifdef ABORT_AT_FAIL
			Abort("locator_create_prefetch_desc, failed to open locator partition file");
#else
				  ...
				  List *hap_partid_list)
{
	 ...
	 * allocate and initialize scan descriptor
	 */
	pscan = (LocatorScanDesc) palloc(sizeof(LocatorScanDescData));
			...
			for (int i = max_level - 1; i >= 0; --i)
			{
				...
				if (prev_part_nums[i] != curr_part_num)
				{
					if (i == 0)
					{
						...
						pscan->level_zero_part_nums[pscan->level_zero_part_cnt] =
																 curr_part_num;
						pscan->level_zero_part_cnt += 1;
					}
					else
					{
	...
	{
		scan->rs_base.rs_prefetch
			= locator_create_prefetch_desc(relation, pscan);
	}

	pscan->locator_columnar_layout_scan_started = false;
	pscan->c_rec_position = 0;
...
locator_rescan(TableScanDesc sscan, ScanKey key, bool set_params,
			   bool allow_strat, bool allow_sync, bool allow_pagemode)
{
	LocatorScanDesc pscan = (LocatorScanDesc) sscan;
	HeapScanDesc scan = (HeapScanDesc) sscan;
	PrefetchDesc prefetchDesc = sscan->rs_prefetch;
	...
	if (part_info != NULL)
	{
		fprintf(stderr, "[SeqScan] rel: %u, level: %u, partNum: %u, partGen: %u, recCnt: %u, scanned records: %u, valid versions: %u, pid: %d\n",
						sscan->rs_rd->rd_node.relNode, pscan->c_level, part_info->partNum, part_info->partGen,
						part_info->recCnt, pscan->c_scanned_records, pscan->c_valid_versions, getpid());
	}
#endif /* LOCATOR_DEBUG */
 ...
 */
static void
locatorsetpage(LocatorScanDesc pscan)
{
	List *c_list;
	ListCell *c_lc;
	...
	fprintf(stderr, "[SeqScan] rel: %u, level: %u, partNum: %u, partGen: %u, recCnt: %u, scanned records: %u, valid versions: %u, pid: %d\n",
					pscan->base.rs_base.rs_rd->rd_id, c_level, c_part_info->partNum, c_part_info->partGen,
					c_part_info->recCnt, pscan->c_scanned_records, pscan->c_valid_versions, getpid());
...
#ifdef LOCATOR_DEBUG
	pscan->c_scanned_records += lines / 3;
#endif /* LOCATOR_DEBUG */
 ...
 */
static void
locatorgettup(LocatorScanDesc pscan,
			  ScanDirection dir,
			  int nkeys,
			  ScanKey key,
			  LocatorExecutor *locator_executor)
{
	...
	if (!scan->rs_inited)
	{
		 ...
		 */
		if (locator_is_columnar_layout(locator_executor, pscan->c_level))
		{
			if (BufferIsValid(scan->rs_cbuf))
			{
				ReleaseBuffer(scan->rs_cbuf);
			...
			pscan->locator_columnar_layout_scan_started = true;
			pscan->c_rec_position = 0;
			return;
		}

		locatorgetpage((TableScanDesc) pscan, false, NULL, NULL);
 ...
 */
static void
locatorgettup_pagemode(LocatorScanDesc pscan,
					   ScanDirection dir,
					   int nkeys,
					   ScanKey key,
					   LocatorExecutor *locator_executor)
...
locator_getnext(TableScanDesc sscan, ScanDirection direction)
{
	...
	if (pagemode)
		locatorgettup_pagemode(pscan, direction,
				   			   scan->rs_base.rs_nkeys, scan->rs_base.rs_key,
							   NULL);
	else
		locatorgettup(pscan, direction,
					  scan->rs_base.rs_nkeys, scan->rs_base.rs_key,
					  NULL);
 ...
 */
static void
locator_init_prefetch_desc(LocatorScanDesc pscan,
						   Relation rel,
						   LocatorExecutorLevelDesc *level_desc)
{
	ListCell *lc;
	...
	foreach(lc, level_desc->required_column_group_desc_list)
	{
		...
		PrefetchDesc prefetch_desc = locator_create_prefetch_desc(rel, pscan);
 ...
 */
static void
locator_set_partition_to_scan(LocatorScanDesc pscan,
							  LocatorExecutor *locator_executor)
{
	Relation rel = pscan->base.rs_base.rs_rd;
	LocatorPartInfo next_part_info;
	ListCell *next_part_info_lc;
	List *part_info_list;

#ifdef LOCATOR_DEBUG
	Assert(pscan->c_part_info != NULL);
#endif /* LOCATOR_DEBUG */

	/* 
	 * This partition has not yet been fully read. No need to move.
	 */
	if (pscan->c_rec_position < pscan->c_part_info->recCnt)
		return;

	part_info_list = pscan->part_infos_to_scan[pscan->c_level];
	next_part_info_lc = lnext(part_info_list, pscan->c_lc);
	...
	if (next_part_info_lc == NULL)
	{
		 ...
		 */
		if (pscan->c_level == rel->rd_locator_level_count)
		{
			pscan->c_part_info = NULL;
			return;
		}
	...
	pscan->c_lc = next_part_info_lc;
	pscan->c_part_info = next_part_info;
	pscan->c_rec_position = 0;
}
...
move_partition:
	 ...
	 */
	locator_set_partition_to_scan(pscan, executor);

	/* There is no partition to read anymore */
	if (pscan->c_part_info == NULL)
	{
	 ...
	 */
	level_desc = locator_get_level_columnar_desc(executor, pscan->c_level);

	if (!level_desc->prefetch_inited && enable_prefetch)
		locator_init_prefetch_desc(pscan, executor->rel, level_desc);
					...
					TupleTableSlot *slot)
{
...
#ifdef LOCATOR_DEBUG
	Oid relid = pscan->base.rs_base.rs_rd->rd_id;
	...
	if (pscan->locator_columnar_layout_scan_started)
	 ...
	 */
	if (unlikely(pscan->locator_columnar_layout_scan_started))
		return locator_columnar_getnextslot(sscan, slot, pagemode);
